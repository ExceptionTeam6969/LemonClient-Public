...
public class SurroundPlus extends LemonModule {
    ...
    private void onBlock(BlockUpdateEvent event) {
        if (event.oldState.getBlock() != event.newState.getBlock() && !LemonUtils.replaceable(event.pos) && surroundBlocks.contains(event.pos)) {
    ...
    private void onRender(Render3DEvent event) {
        ...
        surroundBlocks.stream().filter(LemonUtils::replaceable).forEach(block -> event.renderer.box(block, sideColor.get(), lineColor.get(), shapeMode.get(), 0));
    ...
    private void setBB() {
        if (!centered && center.get() && mc.player.isOnGround() && (!phaseCenter.get() || !LemonUtils.inside(mc.player, mc.player.getBoundingBox().shrink(0.01, 0.01, 0.01)))) {
    ...
    private boolean validBlock(BlockPos pos) {
        if (!LemonUtils.replaceable(pos)) return false;
