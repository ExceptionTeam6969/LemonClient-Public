                ...
                if (!toPlace.isEmpty()) {
                    ...
                    if (obsidian >= 0) {
                        ...
                        for (int i = 0; i < Math.min(obsidian, toPlace.size()); i++) {
                            ...
                            if (placeData.valid()) {
                                boolean rotated = !SettingUtils.shouldRotate(RotationType.BlockPlace) || Managers.ROTATION.start(placeData.pos(), 1, RotationType.BlockPlace, Objects.hash(name + "placing"));
    ...
    private List<BlockPos> getValid(List<BlockPos> blocks) {
        ...
        blocks.forEach(block -> {
            ...
            for (Direction dir : Direction.values()) {
                if (!LemonUtils.replaceable(block.offset(dir)) || !SettingUtils.inPlaceRange(block.offset(dir))) {
                    continue;
                }
        ...
        if (mc.player != null && mc.world != null) {
            for (int x = size[0] - 1; x <= size[1] + 1; x++) {
                for (int z = size[2] - 1; z <= size[3] + 1; z++) {
                    ...
                    boolean ignore = isX && !isZ ? (!LemonUtils.replaceable(pos.add(LemonUtils.closerToZero(x), 0, z)) || placed.contains(pos.add(LemonUtils.closerToZero(x), 0, z))) :
                        !isX && isZ && (!LemonUtils.replaceable(pos.add(x, 0, LemonUtils.closerToZero(z))) || placed.contains(pos.add(x, 0, LemonUtils.closerToZero(z))));
                    BlockPos bPos = null;
                    if (eye() && isX != isZ && !ignore) {
                        bPos = new BlockPos(x, pos.getY(), z).add(pos.getX(), 0, pos.getZ());
                    } else if (top() && !isX && !isZ && LemonUtils.replaceable(pos.add(x, 0, z)) && !placed.contains(pos.add(x, 0, z))) {
