...
public class AutoHoleFillPlus extends LemonModule {
    ...
    private boolean validPos(BlockPos pos) {
        ...
        if (LemonEntityUtils.intersectsWithEntity(Box.from(new BlockBox(pos)), entity -> !entity.isSpectator() && !(entity instanceof ItemEntity), boxes)) return false;
    ...
    private boolean selfNearCheck(Hole hole) {
        ...
        if (iSelfHole.get() && (HoleUtils.inHole(mc.player) || LemonUtils.collidable(pos))) return false;
    ...
    private boolean nearCheck(AbstractClientPlayerEntity player, Hole hole, double pDist) {
        ...
        if ((HoleUtils.inHole(player) || LemonUtils.collidable(pos)) && iHole.get()) return false;
