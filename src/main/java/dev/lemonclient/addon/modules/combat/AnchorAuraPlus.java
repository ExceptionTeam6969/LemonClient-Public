...
public class AnchorAuraPlus extends LemonModule {
        ...
        for (int x = -i; x <= i; x++) {
            for (int y = -i; y <= i; y++) {
                for (int z = -i; z <= i; z++) {
                    ...
                    if (!LemonUtils.replaceable(pos) && !(mc.world.getBlockState(pos).getBlock() == Blocks.RESPAWN_ANCHOR)) continue;
    ...
    private void explode() {
        ...
        if (glowHand == null) {
            switch (switchMode.get()) {
                ...
                case PickSilent -> switched = InventoryUtils.pickSwitch(glowResult.slot());
                case InvSwitch -> switched = InventoryUtils.invSwitch(glowResult.slot());
            }
        }
        ...
        if (explodeHand == null) {
            switch (switchMode.get()) {
                ...
                case PickSilent -> switched = InventoryUtils.pickSwitch(explodeResult.slot());
                case InvSwitch -> switched = InventoryUtils.invSwitch(explodeResult.slot());
            }
        }
    ...
    private void getDmg(BlockPos pos) {
        ...
        targets.forEach(target -> {
            double d = DamageInfo.anchorDamage(target, extMap.containsKey(target) ? extMap.get(target) : target.getBoundingBox(), pos);
