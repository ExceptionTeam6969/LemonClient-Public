...
public class AutoMine extends LemonModule {
    ...
    private boolean placeCrystal() {
        ...
        if (!switched) {
            switch (crystalSwitchMode.get()) {
                ...
                case PickSilent -> switched = InventoryUtils.pickSwitch(crystalSlot);
                case InvSwitch -> switched = InventoryUtils.invSwitch(crystalSlot);
            }
        }
    ...
    private Target getCev() {
        ...
        for (AbstractClientPlayerEntity player : enemies) {
            ...
            if (!SettingUtils.inAttackRange(LemonUtils.getCrystalBox(pos.up()))) {
                continue;
            }
    ...
    private Target getSurroundMiner() {
        ...
        for (AbstractClientPlayerEntity player : enemies) {
            for (Direction dir : Direction.Type.HORIZONTAL) {
                ...
                if (((!civ || !pos.equals(civPos)) && !LemonUtils.solid2(pos)) || getBlock(pos) == Blocks.BEDROCK) {
                    continue;
                }
    ...
    private Target getAntiBurrow() {
        ...
        for (AbstractClientPlayerEntity player : enemies) {
            ...
            if (!LemonUtils.solid2(pos) || getBlock(pos) == Blocks.BEDROCK) {
                continue;
            }
