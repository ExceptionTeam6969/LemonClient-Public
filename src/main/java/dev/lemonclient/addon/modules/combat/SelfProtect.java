...
public class SelfProtect extends LemonModule {
    ...
    private void onBlock(BlockUpdateEvent event) {
        if (event.oldState.getBlock() != event.newState.getBlock() && !LemonUtils.replaceable(event.pos) && placePositions.contains(event.pos)) {
    ...
    private void onSlowTickPre(TickEvent.Pre event) {
        ...
        if (tntAura.get()) {
            if (mc.world.getBlockState(top).getBlock().equals(Blocks.TNT)) {
                ...
                if (SettingUtils.shouldRotate(RotationType.BlockPlace)) Managers.ROTATION.start(top, 50, RotationType.BlockPlace, Objects.hash(name + "placing"));
                BlockUtils.place(top, InvUtils.findInHotbar(Items.OBSIDIAN), 50, true);
                tntAured = false;
            }
        }
            ...
            while (true) {
                while (true) {
                    ...
                    if (crystal instanceof EndCrystalEntity && crystal.getBlockPos().equals(top.up())) {
                        ...
                        if (SettingUtils.shouldRotate(RotationType.BlockPlace)) Managers.ROTATION.start(top.up(), priority, RotationType.BlockPlace, Objects.hash(name +"placing"));
                        BlockUtils.place(top.up(), InvUtils.findInHotbar(Items.OBSIDIAN), 50, true);
                        ceved = false;
                    }
                }
            }
    ...
    private void onRender(Render3DEvent event) {
        ...
        mining.removeIf(m -> System.currentTimeMillis() > m.time + maxMineTime.get() * 1000 || (mineStart != null && m.id == mineStart.id) || !LemonUtils.solid2(m.pos));
        ...
        switch (p.type) {
            case 0, 1 -> {
                for (Direction dir : Direction.values()) {
                    ...
                    if (!LemonUtils.replaceable(pos)) continue;
            ...
            case 2, 3 -> {
                ...
                if (!LemonUtils.replaceable(pos)) return;
    ...
    private void place(BlockPos pos) {
        ...
        if (!switched && hand == null) {
            switch (switchMode.get()) {
                ...
                case PickSilent -> switched = InventoryUtils.pickSwitch(result.slot());
                case InvSwitch -> switched = InventoryUtils.invSwitch(result.slot());
            }
        }
    ...
    private boolean shouldProtect(ProtectBlock p) {
        ...
        switch (p.type) {
            case 1 -> {
                if (!LemonUtils.solid2(pos) || mc.world.getBlockState(pos).getBlock() == Blocks.BEDROCK) return false;
            }
            case 2, 3 -> {
