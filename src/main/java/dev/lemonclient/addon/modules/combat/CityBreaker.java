...
public class CityBreaker extends LemonModule {
    ...
    public CityBreaker() {
        super(LemonClient.Combat, "City Breaker", "Automatically breaks target's city block.");
    }
    ...
    private void onTick(final TickEvent.Pre event) {
        ...
        if (!antiSpam) {
            info("breaking " + target.getGameProfile().getName() +" surround");
            if (support.get()) {
                BlockPos downPos = blockPosTarget.down(1);
                Managers.ROTATION.start(downPos, priority, RotationType.BlockPlace, Objects.hash(name + "placing"));
                BlockUtils.place(downPos, InvUtils.findInHotbar(Items.OBSIDIAN), 0, true);
            }
            if ((autoSwapSetting.get() == swapMode.Normal)) pickSwap();

            if (SettingUtils.shouldRotate(RotationType.Mining)) {
                Managers.ROTATION.start(blockPosTarget, priority, RotationType.Mining, Objects.hash(name + "placing"));
                mc.getNetworkHandler().sendPacket(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.START_DESTROY_BLOCK, blockPosTarget, Direction.UP));
            ...
            mc.getNetworkHandler().sendPacket(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.STOP_DESTROY_BLOCK, blockPosTarget, Direction.UP));
            shouldCount = true;
            antiSpam = true;
        }
    ...
    private void onRender2D(final Render2DEvent event) {
        ...
        final Vector3d pos = new Vector3d(blockPosTarget.getX() + 0.5, blockPosTarget.getY() + 0.5,blockPosTarget.getZ() + 0.5);
