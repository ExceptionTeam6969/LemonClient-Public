...
import static dev.lemonclient.addon.utils.entity.EntityInfo.*;
import static dev.lemonclient.addon.utils.misc.Vec3dInfo.closestVec3d;
import static dev.lemonclient.addon.utils.player.BedUtils.*;
...
public class BedBombV2 extends LemonModule {
    ...
    // PvP
    private final Setting<Boolean> lay = sgPvP.add(new BoolSetting.Builder().name("auto-lay").description("Esu enemy.").defaultValue(true).build());
    ...
    private final Setting<SettingColor> textColor = sgRender.add(new ColorSetting.Builder().name("Text Color").description("The text color for positions to be mined.").defaultValue(new SettingColor(255, 0, 170, 10)).build());

    // Notifications
    private final Setting<Notifications.Mode> notifications = sgNone.add(new EnumSetting.Builder<Notifications.Mode>().name("Notifications").defaultValue(Notifications.Mode.Chat).build());
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (pauseOnCA.get() && (Modules.get().get(CrystalAura.class).isActive() || Modules.get().get(AutoCrystal.class).isActive() || Modules.get().get(AutoCrystalPlus.class).isActive()))
            return;
        ...
        if (bBreakerMain.get()
            ...
            infoTask.run(() -> {
                ...
                switch (notifications.get()) {
                    case Chat -> warning("Burrow Breaker triggered!");
                    case Notification -> Managers.NOTIFICATION.warn(title, "Burrow Breaker triggered!");
                    case Toast -> ToastNotifications.addToast("Burrow Breaker triggered!");
                }
            });
            ...
            if (tBreakerMain.get()
                ...
                infoTask.run(() -> {
                    ...
                    switch (notifications.get()) {
                        case Chat -> warning("Trap Breaker triggered!");
                        case Notification -> Managers.NOTIFICATION.warn(title, "Trap Breaker triggered!");
                        case Toast -> ToastNotifications.addToast("Trap Breaker triggered!");
                    }
                });
                ...
                if (sBreakerMain.get()
                    ...
                    for (CardinalDirection dir : CardinalDirection.values()) {
                        ...
                        if (mc.world.getBlockState(cPos.offset(dir.toDirection())).getBlock().asItem().equals(Items.STRING) && mc.player.getPos().distanceTo(getCenterVec3d(cPos.offset(dir.toDirection()))) < 4.5)
                    ...
                    if (!strings.isEmpty()) {
                        infoTask.run(() -> {
                            switch (notifications.get()) {
                                ...
                                case Toast -> ToastNotifications.addToast("String Breaker triggered!");
                            }
                        });
