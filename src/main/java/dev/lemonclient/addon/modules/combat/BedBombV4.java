...
public class BedBombV4 extends LemonModule {
    ...
    private void calculate(int index) {
        ...
        for (int i = lastIndex; i < index; i++) {
            ...
            for (Direction dir : Direction.Type.HORIZONTAL) {
                ...
                if (!LemonUtils.replaceable(pos.offset(dir)) && !(mc.world.getBlockState(pos.offset(dir)).getBlock() instanceof BedBlock))
                    continue;
        ...
        for (int x = -i; x <= i; x++) {
            for (int y = -i; y <= i; y++) {
                for (int z = -i; z <= i; z++) {
                    ...
                    if (!LemonUtils.replaceable(pos) && !(mc.world.getBlockState(pos).getBlock() instanceof BedBlock))
                        continue;
    ...
    private void update() {
        ...
        if (logicMode.get() == LogicMode.PlaceBreak) {
            ...
            if (LemonUtils.replaceable(placePos) && LemonUtils.replaceable(placePos.offset(bedDir)) && placeUpdate()) {
                removeBed2(placePos);
