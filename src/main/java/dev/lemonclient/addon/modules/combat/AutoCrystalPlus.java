...
public class AutoCrystalPlus extends LemonModule {
    ...
    private void onSend(PacketEvent.Send event) {
        if (mc.player != null && mc.world != null) {
            ...
            if (event.packet instanceof PlayerInteractBlockC2SPacket packet) {
                ...
                blocked.add(LemonUtils.getCrystalBox(packet.getBlockHitResult().getBlockPos().up()));
    ...
    private void update() {
        ...
        switch (switchMode.get()) {
            ...
            case Gapple -> {
                int gapSlot = InvUtils.findInHotbar(LemonUtils::isGapple).slot();
                if (mc.options.useKey.isPressed() && Managers.HOLDING.isHolding(Items.END_CRYSTAL, Items.ENCHANTED_GOLDEN_APPLE, Items.GOLDEN_APPLE) && gapSlot >= 0) {
                    if (getHand(LemonUtils::isGapple) == null) {
    ...
    private void placeCrystal(BlockPos pos, Direction dir, Hand handToUse, int sl, int hsl) {
        if (pos != null && mc.player != null) {
            ...
            if (switched) {
                switch (switchMode.get()) {
                    case PickSilent -> InventoryUtils.pickSwitch(sl);
                    case Silent -> InvUtils.swap(hsl, true);
                    case InvSilent -> InventoryUtils.invSwitch(sl);
                }
            }
            ...
            if (switched) {
                switch (switchMode.get()) {
                    ...
                    case InvSilent -> InventoryUtils.swapBack();
                }
            }
            if (idPredict.get()) {
    ...
    private void explode(int id, Vec3d vec) {
        attackEntity(id, LemonUtils.getCrystalBox(vec), vec);
    }
