...
import static dev.lemonclient.addon.utils.world.BlockInfo.*;
...
public class NewSurround extends LemonModule {
    ...
    private final Setting<Boolean> helper = sgGeneral.add(new BoolSetting.Builder().name("Helper").description("Help to Place Surround.").defaultValue(false).build());
    private final Setting<Boolean> packet = sgGeneral.add(new BoolSetting.Builder().name("packet").description("Packet block placing method.").defaultValue(false).build());
...
//    private final Setting<Boolean> rotate = sgGeneral.add(new BoolSetting.Builder().name("rotate").description("Automatically faces towards the obsidian being placed.").defaultValue(false).build());
    ...
    private final Setting<Boolean> antiCrystal = sgSafe.add(new BoolSetting.Builder().name("anti-crystal").description("Destroys all nearby crystals which can block surround blocks.").defaultValue(true).build());
    ...
    private final Setting<Boolean> antiCrystalBlockSwap = sgSafe.add(new BoolSetting.Builder().name("block-swap").description("Swapping to obsidian block while breaking crystal(for servers with break delay after switch)").defaultValue(false).visible(antiCrystal::get).build());
    //center
    ...
    private final Setting<SettingColor> sideColor2 = sgRender.add(new ColorSetting.Builder().name("second-side-color").description("Side color.").defaultValue(new SettingColor(0, 170, 255, 25)).visible(newRender::get).build());
    private final Setting<SettingColor> lineColor2 = sgRender.add(new ColorSetting.Builder().name("second-line-color").description("Line color.").defaultValue(new SettingColor(0, 170, 255, 55)).visible(newRender::get).build());
    private final Setting<ShapeMode> shapeMode = sgRender.add(new EnumSetting.Builder<ShapeMode>().name("shape-mode").description("How the shapes are rendered.").defaultValue(ShapeMode.Both).build());
    private final Setting<SettingColor> sideColor = sgRender.add(new ColorSetting.Builder().name("side-color").description("Side color.").defaultValue(new SettingColor(0, 170, 255, 25)).build());
    private final Setting<SettingColor> lineColor = sgRender.add(new ColorSetting.Builder().name("line-color").description("Line color.").defaultValue(new SettingColor(0, 170, 255, 55)).build());
                ...
                if (antiCrystal.get()) {
                    ...
                    for (Entity crstal : mc.world.getOtherEntities(null, pBox, ePr)) {
                        if (mc.player.distanceTo(crstal) <= 2.6) {
                            ...
                            Managers.ROTATION.start(crstal.getBoundingBox(), priority, RotationType.Attacking, Objects.hash(name + "attacking"));
        ...
        for (Direction side : Direction.values()) {
            ...
            if (forceSurround.get()) {
                if (packet.get()) {
                    ...
                    packetPlace(blockPos.offset(side));
                    InvUtils.swap(prevSlot, false);
                } else
                    if (SettingUtils.shouldRotate(RotationType.BlockPlace)) Managers.ROTATION.start(blockPos.offset(side), priority, RotationType.BlockPlace, Objects.hash(name +"placing"));
                    BlockUtils.place(blockPos.offset(side), InvUtils.findInHotbar(itemStack -> blocks.get().contains(Block.getBlockFromItem(itemStack.getItem()))), false, 20, swing.get(), false);
        ...
        for (BlockPos p : posPlaceBlocks) {
            if (BlockUtils.canPlace(p, false) && ticks <= bpt.get()) {
                ...
                if (packet.get()) {
                    ...
                    if (SettingUtils.shouldRotate(RotationType.BlockPlace)) Managers.ROTATION.start(p, priority, RotationType.BlockPlace, Objects.hash(name + "placing"));
                    BlockUtils.place(p, InvUtils.findInHotbar(itemStack -> blocks.get().contains(Block.getBlockFromItem(itemStack.getItem()))), false, 20, swing.get(), false);
                if (AGB.get())
    ...
    private void packetPlace(BlockPos blockPos) {
        if (!BlockUtils.canPlace(blockPos, false)) return;
        ...
        if (SettingUtils.shouldRotate(RotationType.BlockPlace)) {
            Managers.ROTATION.start(Box.from(hitPos), 20, RotationType.BlockPlace, Objects.hash(name + "placing"));
    ...
    private boolean inCenter() {
        ...
        int count = 0;
        if (mc.player.getBlockPos().equals(BlockPos.ofFloored(mc.player.getX() - (mc.player.getWidth() + 0.1) / 2.0, mc.player.getY(), mc.player.getZ() - (mc.player.getWidth() + 0.1) / 2.0))) {
            count++;
        }
        if (mc.player.getBlockPos().equals(BlockPos.ofFloored(mc.player.getX() + (mc.player.getWidth() + 0.1) / 2.0, mc.player.getY(), mc.player.getZ() + (mc.player.getWidth() + 0.1) / 2.0))) {
            count++;
