...
public class SpeedMine extends LemonModule {
    ...
    public void onTick(TickEvent.Post event) {
        ...
        if (bypass.get().equals(BypassMode.Auto) && !BlockInfo.isAir(blockPos) && BlockInfo.distanceTo(blockPos) < 5) {
            ...
            if (mc.world.isAir(blockPos)) mineTimer.reset();
    ...
    private void swap(int slot, BlockPos pos, boolean crystal, boolean check) {
        if (check && (slot == -1 || BlockInfo.progress < 1 || (ignoreAir.get() && BlockInfo.isAir(blockPos)) || (!instant.get() && breakTimes >= 1) || !timer.passedTicks(actionDelay.get())))
            return;
