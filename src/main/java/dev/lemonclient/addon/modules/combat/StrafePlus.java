...
import static dev.lemonclient.addon.utils.timers.TimerUtils.getTPSMatch;
...
public class StrafePlus extends LemonModule {
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (BlockInfo.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && webbedPause.get() == WebbedPause.Always) return;
        if (BlockInfo.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && !mc.player.isOnGround() && webbedPause.get() == WebbedPause.OnAir) return;
