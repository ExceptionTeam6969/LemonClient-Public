...
public class PistonCrystal extends LemonModule {
    ...
    private void updateAttack() {
        ...
        if (SettingUtils.shouldRotate(RotationType.Attacking) && !Managers.ROTATION.start(crystal.getBoundingBox(), priority - 0.1, RotationType.Attacking, Objects.hash(name + "attack"))) return;
    ...
    private void updatePiston() {
        ...
        if (hand == null) {
            switch (pistonSwitch.get()) {
                ...
                case PickSilent -> switched = InventoryUtils.pickSwitch(InvUtils.find(Items.PISTON).slot());
                case InvSwitch -> switched = InventoryUtils.invSwitch(InvUtils.find(Items.PISTON).slot());
            }
        }
    ...
    private void updateCrystal() {
        ...
        if (hand == null) {
            switch (crystalSwitch.get()) {
                ...
                case PickSilent -> switched = InventoryUtils.pickSwitch(InvUtils.find(Items.END_CRYSTAL).slot());
                case InvSwitch -> switched = InventoryUtils.invSwitch(InvUtils.find(Items.END_CRYSTAL).slot());
            }
        }
    ...
    private void updateRedstone() {
        ...
        if (SettingUtils.shouldRotate(RotationType.BlockPlace) && !Managers.ROTATION.start(redstoneData.pos(), priority, RotationType.BlockPlace, Objects.hash(name + "restone"))) return;
        ...
        if (hand == null) {
            switch (redstoneSwitch.get()) {
                ...
                case PickSilent -> switched = InventoryUtils.pickSwitch(InvUtils.find(redstone.get().i).slot());
                case InvSwitch -> switched = InventoryUtils.invSwitch(InvUtils.find(redstone.get().i).slot());
            }
        }
    ...
    private void updateFire() {
        ...
        for (int x = (crystalDir.getOpposite().getOffsetX() == 0 ? -1 : Math.min(0, crystalDir.getOffsetX())); x <= (crystalDir.getOpposite().getOffsetX() == 0 ? 1 : Math.max(0, crystalDir.getOpposite().getOffsetX())); x++) {
            for (int y = 0; y <= 1; y++) {
                for (int z = (crystalDir.getOpposite().getOffsetZ() == 0 ? -1 : Math.min(0, crystalDir.getOffsetZ())); z <= (crystalDir.getOpposite().getOffsetZ() == 0 ? 1 : Math.max(0, crystalDir.getOpposite().getOffsetZ())); z++) {
                    ...
                    if (!LemonUtils.solid(pos.down())) continue;
        ...
        if (hand == null) {
            switch (fireSwitch.get()) {
                ...
                case PickSilent -> switched = InventoryUtils.pickSwitch(InvUtils.find(Items.FLINT_AND_STEEL).slot());
                case InvSwitch -> switched = InventoryUtils.invSwitch(InvUtils.find(Items.FLINT_AND_STEEL).slot());
            }
        }
    ...
    private List<BlockPos> pistonBlocks(BlockPos pos, Direction dir) {
        ...
        return blocks.stream().filter(b -> {
            ...
            return LemonUtils.replaceable(b);
        }).toList();
    }
    ...
    private void redstonePos(BlockPos pos, Direction pDir, BlockPos cPos) {
        ...
        if (redstone.get() == Redstone.Torch) {
            for (Direction direction : Direction.values()) {
                ...
                if (!LemonUtils.replaceable(position) && !(mc.world.getBlockState(position).getBlock() instanceof RedstoneTorchBlock) && !(mc.world.getBlockState(position).getBlock() instanceof FireBlock)) {
                    continue;
                }

                redstoneData = SettingUtils.getPlaceDataAND(position, d -> {
                    if (d == Direction.UP && !LemonUtils.solid(position.down())) {
                        return false;
                    }
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (!LemonUtils.replaceable(position) && mc.world.getBlockState(position).getBlock() != Blocks.REDSTONE_BLOCK) {
                continue;
            }
            if (Box.from(new BlockBox(position)).intersects(LemonUtils.getCrystalBox(cPos))) {
                continue;
            }
