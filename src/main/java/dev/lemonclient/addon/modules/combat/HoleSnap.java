        ...
        if (mc.player != null && mc.world != null) {
            ...
            if (hole != null && !singleBlocked()) {
                ...
                if (mc.player.getX() == hole.middle.x && mc.player.getZ() == hole.middle.z) {
                    ...
                    } else if (LemonUtils.inside(mc.player, mc.player.getBoundingBox().offset(0, -0.05, 0))) {
                        this.toggle();
                    ...
                    if (LemonUtils.inside(mc.player, mc.player.getBoundingBox().offset(x, 0, z))) {
                        collisions++;
                    ...
                    if (ticks > 0) {
                        ticks--;
                    } else if (LemonUtils.inside(mc.player, mc.player.getBoundingBox().offset(0, -0.05, 0)) && jump.get()) {
    ...
    private void onRender3D(Render3DEvent event) {
        if (render.get()) {
            ...
            if (targetHole != null) {
                ...
                int iPath = event.renderer.lines.vec3(linePath.x, linePath.y, linePath.z).color(color.get().a(alpha)).next();
                int iPlayer = event.renderer.lines.vec3(from.x, from.y, from.z).color(color.get().a(alpha)).next();
                int iHole = event.renderer.lines.vec3(to.x, to.y, to.z).color(color.get().a(alpha)).next();
