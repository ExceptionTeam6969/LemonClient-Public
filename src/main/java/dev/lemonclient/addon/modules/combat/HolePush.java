...
public class HolePush extends LemonModule {
    ...
    private void update() {
        ...
        for (AbstractClientPlayerEntity player : mc.world.getPlayers()) {
            ...
            if (!LemonUtils.solid2(player.getBlockPos()) && onlyHole.get() && HoleUtils.getHole(player.getBlockPos(), true, true, false, 1, true).type == HoleType.NotHole) return;
    ...
    private void updatePos(PlayerEntity player) {
        ...
        if (LemonUtils.solid2(eyePos.up())) return;
        ...
        for (Direction dir : Direction.Type.HORIZONTAL.stream().sorted(Comparator.comparingDouble(d -> eyePos.offset(d).toCenterPos().distanceTo(mc.player.getEyePos()))).toList()) {
            ...
            if (!LemonUtils.replaceable(pos) && !(mc.world.getBlockState(pos).getBlock() instanceof PistonBlock) && mc.world.getBlockState(pos).getBlock() != Blocks.MOVING_PISTON) continue;
            if (LemonUtils.solid2(eyePos.offset(dir.getOpposite()))) continue;
            if (LemonUtils.solid2(eyePos.offset(dir.getOpposite()).up())) continue;
            if (!LemonUtils.solid2(eyePos.offset(dir.getOpposite()).down())) continue;
    ...
    private void updateRedstone(BlockPos pos) {
        if (redstone.get() == Redstone.Torch) {
            for (Direction direction : Arrays.stream(Direction.values()).sorted(Comparator.comparingDouble(i -> pos.offset(i).toCenterPos().distanceTo(mc.player.getEyePos()))).toList()) {
                ...
                if (!LemonUtils.replaceable(position) && !(mc.world.getBlockState(position).getBlock() instanceof RedstoneTorchBlock)) {
                    continue;
                }
