...
public class RangeSettings extends LemonModule {
    ...
    public double placeRangeTo(BlockPos pos, Vec3d from) {
        ...
        switch (placeRangeMode.get()) {
            ...
            case Vanilla -> {
                return getRange(from, LemonUtils.getClosest(mc.player.getEyePos(), feet, 1, 1));
            }
            case CustomBox -> {
                return getRange(from, LemonUtils.getClosest(mc.player.getEyePos(), feet, blockWidth.get(), blockHeight.get()));
            }
        }
        return -1;
    }
    ...
    public double attackRangeTo(Box bb, Vec3d feet, Vec3d from, boolean countReduce) {
        ...
        double dist = switch (attackRangeMode.get()) {
            ...
            case Vanilla ->
                getRange(from, LemonUtils.getClosest(mc.player.getEyePos(), feet, Math.abs(bb.minX - bb.maxX), Math.abs(bb.minY - bb.maxY)));

            case Middle ->
                getRange(from, new Vec3d((bb.minX + bb.maxX) / 2, (bb.minY + bb.maxY) / 2, (bb.minZ + bb.maxZ) / 2));

            case CustomBox ->
                getRange(from, LemonUtils.getClosest(mc.player.getEyePos(), feet, Math.abs(bb.minX - bb.maxX) * closestAttackWidth.get(), Math.abs(bb.minY - bb.maxY) * closestAttackHeight.get()));
    ...
    public double miningRangeTo(BlockPos pos, Vec3d from) {
        ...
        switch (miningRangeMode.get()) {
            ...
            case CustomBox -> {
                return getRange(from, LemonUtils.getClosest(mc.player.getEyePos(), feet, closestMiningWidth.get(), closestMiningHeight.get()));
            }
        }
        return -1;
    }
