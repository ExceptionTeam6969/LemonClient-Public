...
import static dev.lemonclient.addon.utils.Wrapper.randomNum;
import static dev.lemonclient.addon.utils.entity.EntityInfo.getName;
import static dev.lemonclient.addon.utils.entity.EntityInfo.isDead;
import static dev.lemonclient.addon.utils.player.DeathUtils.getTargets;
...
public class AutoEz extends LemonModule {
    ...
    private final Setting<List<String>> popMessages = sgGeneral.add(new StringListSetting.Builder().name("pop-messages").description("Randomly takes the message from the list and sends on target pop.").defaultValue(
        "popped by the best meteor addon lemonclient!",
