...
import dev.lemonclient.addon.ModuleSetting;
...
public class ChatConfig extends ModuleSetting {
    ...
    public final Setting<Mode> mode = sgGeneral.add(new EnumSetting.Builder<Mode>().name("prefix").description("The way to render BedTrap prefix.").defaultValue(Mode.Lemon).build());
    public final Setting<String> text = sgGeneral.add(new StringSetting.Builder().name("text").description("Text of the prefix").defaultValue("LemonClient").build());
    ...
    public ChatConfig() {
        super(LemonClient.InfoCommand, "Chat Config", "The way to render chat messages.");
    }

    @Override
    public void onActivate() {
        if (mode.get() == Mode.Lemon) ChatUtils.registerCustomPrefix("dev.lemonclient.addon", this::getPrefix);
    }
