    ...
    private void onRender(Render3DEvent event) {
        ...
        if (!switched) {
            switch (switchMode.get()) {
                ...
                case PickSilent -> switched = InventoryUtils.pickSwitch(InvUtils.find(Items.ENDER_PEARL).slot());
                case InvSwitch -> switched = InventoryUtils.invSwitch(InvUtils.find(Items.ENDER_PEARL).slot());
            }
        }
    ...
    private boolean cc() {
        ...
        if (hand == null) {
            switch (ccSwitchMode.get()) {
                ...
                case PickSilent ->
                    switched = InventoryUtils.pickSwitch(InvUtils.find(item -> item.getItem() instanceof BlockItem).slot());
                case InvSwitch ->
                    switched = InventoryUtils.invSwitch(InvUtils.find(item -> item.getItem() instanceof BlockItem).slot());
            }
        }
