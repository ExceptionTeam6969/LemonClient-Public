...
public class RotationManager {
    ...
    public boolean start(BlockPos pos, Box box, Vec3d vec, double p, RotationType type, long key) {
        ...
        if (p < priority || (p == priority && (!(target instanceof BoxTarget) || SettingUtils.rotationCheck(((BoxTarget) target).box, type)))) {
            ...
            this.key = key;
            target = pos != null ? new BoxTarget(pos, vec != null ? vec : LemonUtils.getMiddle(box), p, type) : new BoxTarget(box, vec != null ? vec : LemonUtils.getMiddle(box), p, type);
            timer = settings.time(type);
        }
    ...
    public boolean start(Box box, double p, RotationType type, long key) {
        return start(box, LemonUtils.getMiddle(box), p, type, key);
    }
