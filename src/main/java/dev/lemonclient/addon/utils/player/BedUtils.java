...
import static dev.lemonclient.addon.utils.world.PredictionUtils.returnPredictBox;
import static dev.lemonclient.addon.utils.world.PredictionUtils.returnPredictVec;
import static dev.lemonclient.addon.utils.network.PacketUtils.startPacketMine;
...
public class BedUtils {
    ...
    public static boolean shouldBurrowBreak() {
        BlockPos b = getBlockPos(BedBombV2.target);
        return isCombatBlock(b) && mc.player.getPos().distanceTo(getCenterVec3d(b)) < 4.5;
    }

    public static boolean shouldTrapBreak() {
        return isSurrounded(BedBombV2.target) && isTrapped(BedBombV2.target) && getTrapBlock(BedBombV2.target, 4.5) != null;
    }
    ...
    public static boolean shouldStringBreak() {
        ...
        for (CardinalDirection d : CardinalDirection.values()) {
            BlockPos cPos = getBlockPos(BedBombV2.target).up();
